...
Plain CSS — works anywhere. Examples reference React + Tailwind v4 + Radix for the tricky bits; nothing depends on a framework (Tailwind v4: map tokens via `@theme inline`). Fonts: **Syne** + **IBM Plex Mono** + **Noto Sans** (free `@fontsource`); degrades to `system-ui`.
